...
class AirSimProcessor(Processor):
    ...
    def process_observation(self, observation):
        ...
        img2d = np.reshape(img1d, (observation[0].height, observation[0].width))
...
policy = LinearAnnealedPolicy(EpsGreedyQPolicy(), attr='eps', value_max=0.25, value_min=.1, value_test=.05,
                              nb_steps=30000)
...
dqn = DQNAgent(model=model, nb_actions=nb_actions, policy=policy, memory=memory,
               processor=processor, nb_steps_warmup=2000, gamma=.99, target_model_update=10000,
...
if args.mode == 'train':
    ...
    #dqn.load_weights('dqn_BreakoutDeterministic-v4_weights_90000.h5f')
    dqn.fit(env, callbacks=callbacks, nb_steps=90000, log_interval=5000)
